...
//           });            
    ...
    <script>
            <%
                DataCliente cli;
                try {
                    cli = Login.getInfoClienteLogueado(request);
                } catch (Exception ex) {
                    cli = null;
                }
            %>
        $(document).ready(function(){
                $(".datosPed").click(function(){
                    var ped = {
                    ...
                    $("#modalHeader").empty();
                    $("#modalHeader").append(num);
                    $("#modalHeader").append(es);
                    $("#modalHeader").append(nr);
            ...
            <div class="tab-content pull-left">
                ...
                <div class="tab-pane fade in container" id="pedidos">
                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        <th>Detalles</th>
                        <%
                          Set<DataPedido> sdp = cli.getPedidos();
                          Iterator<DataPedido> it = sdp.iterator();
                          while (it.hasNext()) {
                              DataPedido dp = it.next(); %>
                               <tr>
                                  <td id="num"><%=dp.getId()%></td>
                                  <td id="fecha"><%=dp.getFecha().toString()%></td> 
                                  <td id="res"><%=dp.getNickRestaurante()%></td>
                                  <td id="estado"><%=dp.getEstado()%></td>
                                  <td id="precioT"><%="$" + dp.getPrecioTotal()%></td>
                                  <td>
                                      <button type="button" class="btn btn-link datosPed" data-toggle="modal" data-target="#ModalDetalles">Detalles </button>
                                      <br/>
                                      <%if(dp.getEstado() == Estado.Recibido && dp.getComentario() == null){%>
                                      <button type="button" class="btn btn-link botonComentar" data-toggle="modal" data-target="#ModalComentario">Comentar</button>
                                      <%}%>
                                  </td>
                              </tr>
                          <% } %>
